...
//gun array. if IR send codes start interfering, perhaps make codes ordered by damage.Don't go bellow 100ms chamber time. reload runs concurrently with chamber.
Gun guns[] = 
{
  //standard guns
...
const float immuneTime = 68; //works on 68. the ir most likely stays on for 2 frames. having it exact on the time the ir light is on is nesecary since if someone shoots at the same time as an enemy, their immune period will finish before the enemy's shot (given you shoot first). You become immune when shooting or when shot
...
bool chambered = false; //not shooting on spawn
...
void loop()
{
  ...
  //in menu
  if(!active) //may change this to a state int
  {
    HandleSelect();
    HandleSwitchGun();
  }
  ...
  else
  {
    HandleShoot();
    HandleDisarmTrap();
    HandleReload(); //quick reload can be implemented here!
  }
...
void PiezoOff()
{
  ...
  //possibly delete piezooff code and just add notone to timer
  if(!piezoOn)
  {
    NoTone();
  ...
  if(irrecv.decode(&results))
  {
    ...
    if(active && !immune)
    {
      for (int i = 0; i < gunAmount; i++)
      {
        ...
        {
          //should immunity and LED go in a seperate function? (separate from take damage function?)
...
{
  digitalWrite(hitLED, HIGH);
  ActivatePiezo(piezo, 900, deathTime); //
  
  health = 0;
  deaths++;

  //can update screen here because it goes into a delay
  UpdateOled();
...
void DisplayMenu()
{
  ...
  {
    ammo = 0; //needs to be this way, otherwise gun will not be able to arm because it can't reload.
...
{
  //ledcSetup(2, 2000, 8); //setup beeper. change back to 1 if it glitches
